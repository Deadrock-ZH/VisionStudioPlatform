...
namespace Interface2
{
    ...
    public class CameraAPI
    {
        ...
        // 测试自动重连
        MyCamera.MV_CC_DEVICE_INFO device_temp;

        // public static MyCamera.cbOutputExdelegate ImageCallback;
        // 自动重连回调函数
        // MyCamera.cbExceptiondelegate pCallBackFunc;

        uint g_nPayloadSize = 0;                                 // 包大小
        public uint nWidth = 0;                                   // 图像宽
        public uint nHeight = 0;                                  // 图像高
        ...
        /// 图像处理自定义委托
        /// </summary>
        /// <param name="hImage"></param>  // delegate
        ...
        public CameraAPI(string UserID)
        {
            ...
            {
                int nRet;
                strUserID = UserID;                          // 可以掉线重连使用
                bool ifConnected = false;

                // 枚举相机列表
                //  List<MyCamera.MV_CC_DEVICE_INFO> allCameraInfos = null;
